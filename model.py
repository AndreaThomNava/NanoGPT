...
class GPT(nn.Module):
    ...
    def get_num_params(self, non_embedding = True):
        """
        Positional embeddings get substracted.
        The token embeddings would too, except due to the parameter sharing these params
        are actually used as weight in the final layer, so we include them.
        """
    ...
    def forward(self, idx, targets = None):
        ...
        pos_emb = self.transformer.wpe(pos) # positional embeddings of shape (b, t, n_embd)
        x = self.transformer.drop(tok_emb + pos_emb) # randomly drop some tokens
        # pass through each block
        for block in self.transformer.h: # h is a list of blocks
             x = block(x)
        x = self.transformer.ln_f(x) # layer normalization

        if targets is not None:
            # if we are given the desired target compute the loss
            logits = self.lm_head(x) #read-out layer
            loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1), ignore_index=-1)

        else:
            # inference
            logits = self.lm_head(x[:, [-1], :]) # note: using [-1] to preserve the time dim (?)
            loss = None

        return logits, loss
    
    @torch.no_grad() # for inference do not compute the gradient
